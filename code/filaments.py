...
for exp in [0.4, 1, 2, 5]:
    ...
    def get_points(center=[50, 50, 50], radius=25, collapse_exp=2.0, expand_exp=2.0, ids_add=True, n_points=1_000_000):
        ...
        # Collapse in x and y
        extra_xy = -radius * (abs_delta[:, :2] / radius) ** collapse_exp
        end_abs_xy = np.clip(abs_delta[:, :2] + extra_xy, 0.0, None)

        # Expansion in z
        extra_z = radius * (abs_delta[:, 2] / radius) ** expand_exp
        end_abs_z = abs_delta[:, 2] + extra_z

        # Combine collapsed xy and expanded z
    ...
    # Save to files
    filenamepi = f"./results/tests_with_motion_new/filament_pi-nbox{n_boxes}-{n_points}-exp{exp}.csv"
    with open(filenamepi, 'w') as f:
        np.savetxt(f, pi, delimiter=',')

    filenamemunu = f"./results/tests_with_motion_new/filament_munu-nbox{n_boxes}-{n_points}-exp{exp}.csv"

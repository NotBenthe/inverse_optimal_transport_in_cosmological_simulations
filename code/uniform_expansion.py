...
def get_points(center = [25, 25, 25], start_radius = 25, end_radius = 50, n_points = int(1e6), ids_add = True):
...
start, end = get_points(center=[50, 50, 50], start_radius=25, end_radius=50)
boxsize = [100, 100, 100]
...
filenamepi = "./results/tests_with_motion_new/uniform_expansion_pi-nbox" + str(n_boxes) + "-" + str(int(n_points)) + ".csv" 
with open(filenamepi, 'w') as f:
    np.savetxt(f, pi, delimiter=',')

filenamemunu = "./results/tests_with_motion_new/uniform_expansion_munu-nbox" + str(n_boxes) + "-" + str(int(n_points)) + ".csv" 

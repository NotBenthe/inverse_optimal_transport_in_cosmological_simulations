...
for n_boxes in [8]:
    for exp in [0.4, 1, 2, 5]:
        ...
        filenamepi = standpath + begpath + "ma_n" + str(n_boxes) + "_c-" + str(iteration) + ".csv" 
        with open(filenamepi, 'w') as f:
            np.savetxt(f, c, delimiter=',')

        filenamemunu = standpath + begpath + "ma_n" + str(n_boxes) + "_ab-" + str(iteration) + ".csv" 
